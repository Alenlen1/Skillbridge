...
import FindJobsSection from "@/components/tracker/FindJobsSection";
...
export default function TrackerPage() {
  ...
  const [pageMode, setPageMode] = useState<"applications" | "findJobs">(
    "applications",
  );
  ...
  const handleTrackJob = (company: string, role: string) => {
    setForm({ ...EMPTY_FORM, company, role });
    setPageMode("applications");
    setAdding(true);
  };
  ...
  const sortedByLatest = [...applications].sort(
    (a, b) =>
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex gap-1 border-b border-slate-200 dark:border-white/[0.06]">
        <button
          onClick={() => setPageMode("applications")}
      ...
      {pageMode === "findJobs" ? (
        <FindJobsSection onTrack={handleTrackJob} />
      ) : (
        <>
          {error && (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`flex-shrink-0 rounded-full border px-2 py-0.5 text-[11px] font-medium ${STATUS_STYLES[app.status]}`}
                      >
                        {app.status}
                      </span>
                    </div>
                  ...
                  <button
                    onClick={() => handleDelete(app.id)}
                    className="flex-shrink-0 text-slate-700 transition hover:text-red-400"
                  >
                    <IconTrash size={14} stroke={1.5} />
                  </button>
                </div>
